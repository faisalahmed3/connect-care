...
const DonateModal = ({ open, setOpen }) => {
  ...
  const handleConfirm = () => {
    ...
    // Small delay to let DaisyUI clean up
    setTimeout(() => {
      setShowSuccess(true);
    }, 120);
    ...
    <>
      ...
      {open && (
        <dialog className="modal modal-open">
          ...
          >
            ...
            {/* Payment Info */}
            {method && (
              <div className="mt-7 p-4 rounded-xl bg-white/10 border border-white/20 text-white animate-fadeIn">
                {method === "bkash" && (
      ...
      {showSuccess && (
        <div className="fixed inset-0 flex justify-center items-center z-[5000] animate-fadeIn">
          <div className="relative bg-white rounded-2xl shadow-2xl text-center p-10 w-[330px] animate-successPop border border-teal-200">
